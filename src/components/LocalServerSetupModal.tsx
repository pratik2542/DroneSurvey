...
3. A desktop GUI window will open showing your Public Cloudflare Tunnel URL and a "COPY LINK TO CLIPBOARD" button!
4. Click "COPY LINK TO CLIPBOARD" and paste the URL under "CONNECT TILE SERVER" in the Web App!
...
`;
      ...
      // 3. server_app.py (Desktop GUI App)
      const serverAppPy = `#!/usr/bin/env python3
import tkinter as tk
from tkinter import messagebox
import threading, os, subprocess, sys, re, time, urllib.request, json, webbrowser
...
class ServerApp:
    def __init__(self, root: tk.Tk):
        ...
        self.tunnel_url, self.server_process, self.tunnel_process, self.is_running = "", None, None, True
        self._build_ui()
        self._start_backend()
    ...
    def _build_ui(self):
        ...
        tk.Label(card, text='PUBLIC CLOUDFLARE TUNNEL URL', font=(FONT, 9, 'bold'), bg=CARD, fg=SUBTEXT).pack(anchor='w', padx=16, pady=(6, 4))
        ...
        self.url_entry = tk.Entry(url_inner, textvariable=self.url_var, font=('Consolas', 10, 'bold'), bg='#090b14', fg=ACCENT, relief='flat', state='readonly')
        self.url_entry.pack(fill='x', pady=(0, 8))

        btn_row = tk.Frame(url_inner, bg='#090b14')
        btn_row.pack(fill='x')
        self.copy_btn = tk.Button(btn_row, text='📋   COPY LINK TO CLIPBOARD', command=self._copy_url, font=(FONT, 10, 'bold'), bg=SUCCESS, fg='#000000', relief='flat', padx=16, pady=8, cursor='hand2', activebackground='#059669', activeforeground='#ffffff')
        ...
        tk.Label(card, text='LIVE SERVER & TUNNEL LOGS', font=(FONT, 9, 'bold'), bg=CARD, fg=SUBTEXT).pack(anchor='w', padx=16, pady=(10, 4))
        log_wrap = tk.Frame(card, bg='#090b14', padx=1, pady=1)
        log_wrap.pack(fill='both', expand=True, padx=14, pady=(0, 14))
        self.log_box = tk.Text(log_wrap, bg='#090b14', fg=SUCCESS, font=('Consolas', 9), relief='flat', state='disabled', wrap='word')
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-800 space-y-2">
                  ...
                  <h4 className="text-xs font-medium text-white">Run Server App GUI</h4>
                ...
                <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-800 space-y-2">
                  ...
                  <p className="text-xs text-slate-400">Click <span className="text-emerald-400 font-semibold">📋 Copy Link</span> in the desktop GUI and paste it under CONNECT TILE SERVER!</p>
                </div>
              </div>
            </div>
          )}
